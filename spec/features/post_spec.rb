...
describe 'navigate' do
  ...
  describe 'index' do
    ...
    it 'has a scope so that only post creators can see their posts' do
      post1 = Post.create(date: Date.today, rationale: "no need for factories",
                                          user_id: @user.id)
  ...
  describe 'edit' do
    before do
      @edit_user = User.create(first_name: 'test', last_name: 'user', email: 'ideas@examle.com',
                              password: 'password', password_confirmation: 'password')
      login_as(@edit_user, :scope => :user)
      @edit_post = Post.create(date: Date.today, rationale: 'does not matter', user_id: @edit_user.id)
    end
